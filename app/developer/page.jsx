...
function DeveloperInner({ token }) {
  ...
  useEffect(() => {
    ...
    // Only try admin APIs if we have a token
    if (token) {
      fetches.push(
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {isOwner && st && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: feedback.length > 0 ? 16 : 0 }}>
          ...
          {feedback.length > 0 && (
            <>
              <div style={{ ...SECTION_LABEL, marginTop: 8 }}>
...
export default function DeveloperPage() {
  return (
    <StandaloneShell label="How It Works">
