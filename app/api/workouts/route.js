...
export const POST = withAuth(async (req, { supabase, user }) => {
  ...
  if (Array.isArray(body.data)) {
    // Atomic replace via RPC — DELETE source='manual' + INSERT in one transaction.
...
export const PATCH = withAuth(async (req, { supabase, user }) => {
  ...
  if (body.rows !== undefined) {
    ...
    // Atomic replace via RPC — DELETE oura/strava rows + INSERT in one transaction.
    const toInsert = (rows || [])
      .filter(r => r.text?.trim() && r.source !== 'manual')
      .map(r => ({
        title:    r.text,
