...
export async function GET(request) {
  ...
  try {
    ...
    // bedtime_end is an ISO timestamp — extract its local date by comparing to date and date+1.
    // This prevents wide day-window lookups from pulling future sleep onto today's date.
    const mainSession = sessions
      .filter(s => {
        if (!NON_NAP.includes(s.type)) return false;
        if (!s.bedtime_end) {
          // Fallback to day field if no bedtime_end
