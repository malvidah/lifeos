...
// ─── DB helpers (pass JWT so RLS knows who the user is) ───────────────────────
...
function useDbSave(date, type, empty, token) {
  ...
  useEffect(() => {
    ...
    if (key in MEM) {
      _setValue(MEM[key]); live.current = MEM[key]; setLoaded(true); return;
    }
    setLoaded(false); _setValue(empty); live.current = empty;
    dbLoad(date, type, token).then(v => {
      const val = v ?? empty;
      MEM[key] = val; _setValue(val); live.current = val; setLoaded(true);
    });
...
function Ring({ score, color, size = 44 }) {
  ...
  const val = parseFloat(score)||0, pct = Math.min(val/100,1), elite = val>=90;
  return (
...
function RowList({ date, type, placeholder, promptFn, prefix, color, token }) {
  ...
  function onKey(e, id, idx) {
    ...
    if (e.key==="Backspace"&&safe[idx].text===""&&safe.length>1) {
      ...
      setTimeout(()=>inputRefs.current[t]?.focus(),30);
    }
  }
...
function Meals({ date, token }) {
  return <RowList date={date} type="meals" token={token} placeholder="What did you eat?"
    promptFn={t=>`Calories in: "${t}". Return JSON: {"kcal":420}`} prefix="" color={C.accent}/>;
}
function Activity({ date, token }) {
  return <RowList date={date} type="activity" token={token} placeholder="What did you do?"
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ fontFamily:serif, fontSize:28, color:C.text, marginBottom:8, letterSpacing:"-0.02em" }}>
          Life OS
        </div>
        <div style={{ fontFamily:mono, fontSize:9, color:C.dim, letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:40 }}>
...
export default function Dashboard() {
  ...
  const googleToken = session?.provider_token; // Google OAuth token for Calendar
  ...
  // Calendar sync using Google OAuth token from session
  useEffect(() => {
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", color:C.text, display:"flex", flexDirection:"column" }}>
      ...
      <HealthStrip date={selected} token={token} onHealthChange={onHealthChange}
