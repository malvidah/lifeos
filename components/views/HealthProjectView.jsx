...
export default function HealthProjectView({ token, userId, onBack, onHealthChange, onScoresReady, startSync, endSync, onSelectDate, taskFilter, setTaskFilter }) {
  ...
  const mealsWidget = (
    <Widget label="Meals" color={"var(--dl-red)"} autoHeight collapsed={mealsCollapsed} onToggle={toggleMeals}
  ...
  const activitiesWidget = (
    <Widget label="Workouts" color={"var(--dl-green)"} autoHeight collapsed={activitiesCollapsed} onToggle={toggleActivities}
      headerRight={<span style={{display:'flex',gap:0}}>
