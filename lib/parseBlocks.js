...
export function parseTaskBlocks(html) {
  ...
  while (i < html.length) {
    ...
    const inner = liForText
      ...
      .replace(/<span\b[^>]*\bdata-drawing-tag="([^"]*)"[^>]*>[^<]*<\/span>/g, '{d:$1}')
    ...
    // Strip blank nested task items (created by pressing Tab without typing).
    // Pattern: <li data-type="taskItem"> with <div><p></p></div> and nothing else.
    // TipTap emits <label contenteditable="false"> and may add class/data attrs to
    // <div> and <p>, so we use [^>]* throughout rather than bare tag names.
    // Also drop any <ul data-type="taskList"> that becomes empty after stripping.
    const liHtmlClean = liHtml
      .replace(/<li\b[^>]*data-type="taskItem"[^>]*>(?:\s*<label\b[^>]*>[\s\S]*?<\/label>)?\s*<div\b[^>]*>\s*<p\b[^>]*>\s*<\/p>\s*<\/div>\s*<\/li>/g, '')
      .replace(/<ul\b[^>]*data-type="taskList"[^>]*>\s*<\/ul>/g, '');

    tasks.push({
      position:     position++,
      html:         liHtmlClean,   // full html including any nested subtasks (blank subtasks stripped)
      text,
      done,
