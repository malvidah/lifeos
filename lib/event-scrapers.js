...
const PORTLAND_VENUES = {
  ...
  'process pdx': { lat: 45.4730, lng: -122.6555 },
  'white owl social club': { lat: 45.5119, lng: -122.6555 },
  'produce row cafe': { lat: 45.5085, lng: -122.6607 },
  '45 east': { lat: 45.5248, lng: -122.6571 },
  'the den': { lat: 45.5120, lng: -122.6567 },
  'realm': { lat: 45.5227, lng: -122.6814 },
  'wonderlove': { lat: 45.5227, lng: -122.6575 },
  'secret grove': { lat: 45.5120, lng: -122.6555 },
  'newmark theatre': { lat: 45.5159, lng: -122.6818 },
  'mcmenamins grand lodge': { lat: 45.5037, lng: -122.8624 },
  'pam cut tomorrow theatre': { lat: 45.5164, lng: -122.6834 },
  'portland art museum': { lat: 45.5164, lng: -122.6834 },
...
// ─── Portland address grid estimator ─────────────────────────────────────────
// Portland's address grid is centered on Burnside (N/S) and the Willamette (E/W).
// Address numbers increase by ~100 per block away from the axis.

const PDX_LAT = 45.5231;
const PDX_LNG = -122.6765;
const DEG_LAT_PER_ADDR = 0.00001;
const DEG_LNG_PER_ADDR = 0.000013;
const DEG_LNG_PER_AVE = 0.0012;

const NS_STREET_LNG = {
  'naito': -122.6735, 'front': -122.6735, 'water': -122.6695, 'moody': -122.6710,
  'mlk': -122.6608, 'martin luther king': -122.6608, 'grand': -122.6597,
  'milwaukie': -122.6555, 'interstate': -122.6795, 'greeley': -122.6810,
  'williams': -122.6670, 'vancouver': -122.6680, 'mississippi': -122.6752,
  'sandy': -122.6580, 'barbur': -122.6900, 'capitol': -122.6990,
  'macadam': -122.6780, 'cesar chavez': -122.6208, '39th': -122.6208,
};

const EW_STREET_LAT = {
  'burnside': 45.5231, 'stark': 45.5186, 'belmont': 45.5157,
  'hawthorne': 45.5119, 'division': 45.5046, 'clinton': 45.5039,
  'powell': 45.4967, 'holgate': 45.4908, 'foster': 45.4862,
  'woodstock': 45.4792, 'steele': 45.4772, 'bybee': 45.4826,
  'flavel': 45.4732, 'tacoma': 45.4680,
  'alberta': 45.5590, 'killingsworth': 45.5717, 'prescott': 45.5643,
  'fremont': 45.5472, 'knott': 45.5451, 'broadway': 45.5310,
  'glisan': 45.5265, 'halsey': 45.5360, 'lombard': 45.5764,
  'going': 45.5639, 'ainsworth': 45.5685, 'rosa parks': 45.5714,
  'morrison': 45.5170, 'taylor': 45.5142, 'salmon': 45.5153,
  'main': 45.5095, 'madison': 45.5156, 'harrison': 45.5090,
  'lincoln': 45.5070, 'ankeny': 45.5195, 'flanders': 45.5262,
  'couch': 45.5240, 'hoyt': 45.5277, 'irving': 45.5290,
  'johnson': 45.5296, 'lovejoy': 45.5308, 'overton': 45.5330,
  'pettygrove': 45.5342, 'thurman': 45.5380,
  'weidler': 45.5330, 'multnomah': 45.5310,
  'davis': 45.5236, 'everett': 45.5245,
  'duke': 45.4850, 'bush': 45.4870,
  'crystal springs': 45.4810,
  'clay': 45.5079, 'market': 45.5064, 'mill': 45.5052,
  'stephens': 45.5024, 'ivon': 45.5067, 'caruthers': 45.5052,
  'cora': 45.4988, 'rhine': 45.4988,
};

function estimatePortlandCoords(address) {
  if (!address) return null;
  const cleaned = address.split(',')[0].trim();
  const m = cleaned.match(/^(\d+)\s+(N|NE|NW|S|SE|SW)\s+(.+)/i);
  if (!m) return null;

  const addrNum = parseInt(m[1]);
  const dir = m[2].toUpperCase();
  const streetFull = m[3];
  const street = streetFull.toLowerCase()
    .replace(/\b(ave|avenue|st|street|blvd|boulevard|rd|road|dr|drive|ct|way|pkwy|hwy|ln|pl|jr)\b\.?/gi, '')
    .trim();

  const numStreet = street.match(/^(\d+)/);
  const streetNumber = numStreet ? parseInt(numStreet[1]) : null;
  const northSign = dir.includes('N') ? 1 : -1;
  const eastSign = dir.includes('E') ? 1 : (dir.includes('W') ? -1 : 1);

  let lat = PDX_LAT;
  let lng = PDX_LNG;

  const isNS = /\b(ave|avenue)\b/i.test(streetFull) || NS_STREET_LNG[street] !== undefined;

  if (streetNumber) {
    if (isNS) {
      lat += northSign * addrNum * DEG_LAT_PER_ADDR;
      lng += eastSign * streetNumber * DEG_LNG_PER_AVE;
    } else {
      lng += eastSign * addrNum * DEG_LNG_PER_ADDR;
      lat += northSign * streetNumber * DEG_LAT_PER_ADDR * 100;
    }
  } else if (isNS) {
    lat += northSign * addrNum * DEG_LAT_PER_ADDR;
    if (NS_STREET_LNG[street] !== undefined) lng = NS_STREET_LNG[street];
    else lng += eastSign * 15 * DEG_LNG_PER_AVE;
  } else if (EW_STREET_LAT[street] !== undefined) {
    lng += eastSign * addrNum * DEG_LNG_PER_ADDR;
    lat = EW_STREET_LAT[street];
  } else {
    lng += eastSign * addrNum * DEG_LNG_PER_ADDR;
    lat += northSign * 15 * DEG_LAT_PER_ADDR * 100;
  }
 ...
 * Tries: hardcoded venue name → Portland address grid estimation → null.
 */
export function lookupVenueCoords(venue, address) {
  ...
  const estimated = estimatePortlandCoords(address);
