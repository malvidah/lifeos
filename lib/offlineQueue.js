...
export async function replayQueue() {
  ...
  try {
    ...
    for (const item of queue) {
      // Skip empty payloads — don't overwrite server data with nothing
      const d = item.body?.data;
      const blocks = item.body?.blocks;
      const bodyEmpty = (d === null || d === undefined || d === '' ||
        (typeof d === 'object' && !Array.isArray(d) && Object.keys(d).length === 0) ||
        (Array.isArray(d) && d.length === 0)) &&
        (!blocks || (Array.isArray(blocks) && blocks.length === 0));
      if (bodyEmpty && !item.body?.date) continue; // no date = malformed, skip
      if (bodyEmpty) continue; // empty data — don't overwrite server
