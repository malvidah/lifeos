...
export async function fetchWeather(date, lat, lng) {
  ...
  try {
    ...
    const url = (isToday || isFuture)
      ? `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=weather_code,temperature_2m&temperature_unit=fahrenheit&timezone=auto`
      : `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lng}&start_date=${date}&end_date=${date}&daily=weather_code,temperature_2m_max&temperature_unit=fahrenheit&timezone=auto`;
