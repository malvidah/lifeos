...
export function getSunAltitude(date, lat, lng) {
  ...
  // Approximate solar noon from longitude (not timezone-perfect, but close enough for gradients)
...
export async function fetchWeather(date, lat, lng) {
  ...
  try {
    ...
    const url = isToday
      ...
      : `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lng}&start_date=${date}&end_date=${date}&daily=weather_code&timezone=auto`;
    ...
    const data = { code, condition: weatherCodeToCondition(code) };
