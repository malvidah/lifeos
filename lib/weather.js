...
export async function fetchWeather(date, lat, lng) {
  ...
  try {
    ...
    let url;
    if (isToday) {
      // Current real-time data + daily high/low for the day
      url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=weather_code,temperature_2m&daily=temperature_2m_max,temperature_2m_min&temperature_unit=fahrenheit&timezone=auto`;
    } else if (isFuture) {
      // Forecast for a specific future date
      url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&start_date=${date}&end_date=${date}&daily=weather_code,temperature_2m_max,temperature_2m_min&temperature_unit=fahrenheit&timezone=auto`;
    } else {
      // Historical archive
      url = `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lng}&start_date=${date}&end_date=${date}&daily=weather_code,temperature_2m_max,temperature_2m_min&temperature_unit=fahrenheit&timezone=auto`;
    }
    ...
    let code, temperature, tempHigh, tempLow;
    if (isToday) {
      code = json.current?.weather_code ?? 0;
      temperature = json.current?.temperature_2m ?? null;
      tempHigh = json.daily?.temperature_2m_max?.[0] ?? null;
      tempLow  = json.daily?.temperature_2m_min?.[0] ?? null;
    } else {
    ...
    const data = { code, condition: weatherCodeToCondition(code), temperature, tempHigh, tempLow };
...
// ─── Save today's location to DB (fire-and-forget, once per day) ─────────────

export async function saveLocationIfNeeded(token) {
  if (!token) return;
  const today = todayStr();
  const flag = `daylab:loc-saved:${today}`;
  ...
  try {
    await fetch('/api/location', {
      ...
      body: JSON.stringify({ date: today, lat: loc.lat, lng: loc.lng }),
    });
  } catch {}
}
