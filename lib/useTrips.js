...
// Resolve a single trip by name → full trip with stops. Used by note-tagging
// flows that have a trip name (from a `/tr` chip) and need the route data.
// Two fetches: list (to map name → id), then full trip (?id=X). The browser
// cache + Supabase RLS make this cheap; consumers don't need to dedupe.
export function useTripByName(name, token) {
  const [trip, setTrip] = useState(null);
  useEffect(() => {
    if (!name || !token) { setTrip(null); return; }
    let cancelled = false;
    (async () => {
      const list = await api.get('/api/trips', token);
      const match = (list?.trips || []).find(t => t.name?.toLowerCase() === name.toLowerCase());
      if (!match) { if (!cancelled) setTrip(null); return; }
      const full = await api.get(`/api/trips?id=${match.id}`, token);
      if (!cancelled) setTrip(full?.trip ?? null);
    })();
