...
export function getRecurrenceSuggestions(query) {
  ...
  const options = [
    ...
    { label: '1/week',    key: '1pw',      search: '1x per week once weekly 1 per week' },
    { label: '2/week',    key: '2pw',      search: '2x per week twice weekly 2 per week' },
    { label: '3/week',    key: '3pw',      search: '3x per week three times weekly 3 per week' },
    { label: 'M·T·W·R·F', key: 'weekdays', search: 'weekdays weekday mtwrf' },
    { label: 'M·W·F',     key: 'mwf',      search: 'mwf monday wednesday friday' },
    { label: 'T·R',       key: 'tr',       search: 'tth tuesday thursday tr' },
    { label: 'S·U',       key: 'weekends', search: 'weekends weekend su saturday sunday' },
    { label: 'M',  key: 'mon', search: 'monday mon m every' },
    { label: 'T',  key: 'tue', search: 'tuesday tue t every' },
    { label: 'W',  key: 'wed', search: 'wednesday wed w every' },
    { label: 'R',  key: 'thu', search: 'thursday thu r every' },
    { label: 'F',  key: 'fri', search: 'friday fri f every' },
    { label: 'S',  key: 'sat', search: 'saturday sat s every' },
    { label: 'U',  key: 'sun', search: 'sunday sun u every' },
  ];
...
// Extract the weekly target from an xperweek key ('1pw' → 1, '2pw' → 2, etc.)
export function xperweekTarget(key) {
  const m = key?.match(/^(\d+)pw$/);
...
export function keyToRecurrence(key, anchorDate) {
  ...
  if (pwTarget) return { rule: 'xperweek', target: pwTarget };
