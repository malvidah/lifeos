...
/**
 * Get suggestion items for /d autocomplete.
 * Returns items in the format "__recurrence__:rule:data"
 */
export function getRecurrenceSuggestions(query) {
  const q = query.toLowerCase().trim();
  const options = [
    { label: 'Daily', key: 'daily', search: 'daily everyday' },
    { label: 'Weekdays', key: 'weekdays', search: 'weekdays weekday mtwrf' },
    { label: 'MWF', key: 'mwf', search: 'mwf monday wednesday friday' },
    { label: 'Every Monday', key: 'mon', search: 'monday mon m every' },
    { label: 'Every Tuesday', key: 'tue', search: 'tuesday tue t every' },
    { label: 'Every Wednesday', key: 'wed', search: 'wednesday wed w every' },
    { label: 'Every Thursday', key: 'thu', search: 'thursday thu r every' },
    { label: 'Every Friday', key: 'fri', search: 'friday fri f every' },
    { label: 'Every Saturday', key: 'sat', search: 'saturday sat s every' },
    { label: 'Every Sunday', key: 'sun', search: 'sunday sun u every' },
    { label: 'TTh', key: 'tr', search: 'tth tuesday thursday tr' },
    { label: 'Weekends', key: 'weekends', search: 'weekends weekend su' },
  ];
...
export function recurrenceLabel(rec) {
  ...
  if (rec.rule === 'weekly') {
    const days = (rec.days || []).map(d => DNAMES[d]);
    ...
    if (days.length === 1) return `Every ${DNAMES[rec.days[0]]}`;
    return days.join(' · ');
  }
  if (rec.rule === 'biweekly') return `Biweekly ${DNAMES[rec.days?.[0]] || ''}`;
