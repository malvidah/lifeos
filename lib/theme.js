...
export function ThemeProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const ready = el.classList.contains("theme-ready");
    localStorage.setItem("theme", preference); // store preference, not effective
    el.setAttribute("data-theme", effective);
    el.style.colorScheme = effective === "light" ? "light" : "dark";

    if (!ready) {
      // First paint — set inline bg to match, then hand off to CSS vars
    ...
    // After theme-ready, background comes from CSS --dl-bg via data-theme,
    // so no inline style needed — the @property transitions handle the rest.
