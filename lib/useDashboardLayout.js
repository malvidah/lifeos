...
// Generate a stable random page id. Used as a React key so reorders preserve
// component identity and duplicate page names don't collide.
function newPageId() {
  return 'p_' + Math.random().toString(36).slice(2, 10) + Date.now().toString(36);
}

// Backfill stable ids on any pages missing one. Idempotent.
function withIds(arr) {
  return arr.map(p => p?.id ? p : { ...p, id: newPageId() });
}
...
function normalise(raw) {
  if (!raw) return null;
  let arr = null;
  if (Array.isArray(raw)) arr = raw;            // legacy flat array
  else if (raw.shared) arr = raw.shared;        // current format
  else if (raw.desktop) arr = raw.desktop;      // old desktop layout takes precedence
  else if (raw.mobile)  arr = raw.mobile;       // fall back to mobile
...
export function useDashboardLayout(token) {
  ...
  useEffect(() => {
    ...
    api.get('/api/settings', token).then(settings => {
      ...
      if (normalised) {
        ...
        // Rewrite old formats as unified, or persist newly-backfilled page ids
      ...
      // Settings GET failed — show the default layout visually so the app is
      // usable, but do NOT unlock saves (loadedRef stays false). This prevents
      // the fallback DEFAULT_PAGES from being written over the user's real layout
      // in Supabase if the error was transient and the next action triggers a save.
  ...
  const addPage = useCallback((name) => {
    updatePages([...currentPages, { id: newPageId(), name, cards: [] }]);
  }, [currentPages, updatePages]);
