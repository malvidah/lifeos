...
export function useDashboardLayout(token) {
  ...
  // Guards writes so an early user tap (before /api/settings returns) can't
  // fire a save built on DEFAULT_PAGES and overwrite the user's real layout.
  ...
  const save = useCallback((newPages) => {
    if (!loadedRef.current) return; // prevent pre-load writes from wiping saved layout
  ...
  const setHomeIdx = useCallback((idx) => {
    if (!loadedRef.current) return; // same guard as save()
