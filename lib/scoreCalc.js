...
export function calcActivityScore(today, history7d, isToday = false) {
  const steps         = n(today.steps);
  const activeMinutes = n(today.activeMinutes);

  // If this is today and we have no activity data yet, return null — the day
  // isn't done. freq/recovery from history would inflate the score to ~100
  // at e.g. 2 AM when 0 steps have registered. Score populates once data arrives.
  if (isToday && steps == null && activeMinutes == null) {
