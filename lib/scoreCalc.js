...
export function calcSleepScore(today, history) {
  ...
  // Sleep hours score: 7–9h = 100 (optimal range per AASM guidelines).
  // Below 7h: caps at 80 with a ^1.5 curve from 4h, so even 6.9h only reaches ~76.
  // Above 9h: tapers to 0 at 12h (oversleeping penalty).
  // This creates a natural gap where sub-7h sleep can never push the total above ~82.
  //
  // Key values: 6.9h→76, 6.5h→67, 6h→51, 5h→19, 4h→0, 10h→67, 12h→0
  let sleepHrsScore = null;
  if (hrs != null) {
    if (hrs >= 7 && hrs <= 9) sleepHrsScore = 100;
    else if (hrs > 9) sleepHrsScore = clamp(100 - (hrs - 9) * 33);
    else sleepHrsScore = clamp(80 * Math.pow(Math.max(hrs - 4, 0) / 3, 1.5));
  }
  // Efficiency: 85–100% = 100, linear drop to 0 at 50%
  ...
  // Weighting: 75% hours, 25% efficiency. Hours dominate so that
  // high efficiency can't compensate for insufficient sleep.
