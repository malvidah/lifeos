...
export function calcSleepScore(today, history) {
  ...
  // Sleep hours: 7–9h = 100. Under 7h caps at 85 and drops linearly to 0 at 4h.
  // This makes 7h a real threshold, not a soft gradient.
  // 6.9h → 82, 6.5h → 71, 6h → 57, 5h → 28, 4h → 0, >9h tapers to 0 at 12h.
  let sleepHrsScore = null;
  if (hrs != null) {
    if (hrs >= 7 && hrs <= 9) sleepHrsScore = 100;
    else if (hrs > 9) sleepHrsScore = rangeScore(hrs, 7, 9, 5, 12);
    else sleepHrsScore = clamp(85 * Math.max(hrs - 4, 0) / 3);
  }
  ...
  let score = Math.round(weighted.reduce((a, b) => a + b, 0) / totalW);
  // Hard cap: under 7h sleep → total score can't exceed 85 regardless of efficiency
  if (hrs != null && hrs < 7) score = Math.min(score, 85);
  return {
    score,
    contributors: {
