...
//
// The editor is the source of truth. Habit/recurring tasks participate in
// matching, text edits (renames), and deletion — but done-state changes are
// handled separately by the pre-diff toggle in Tasks.jsx (complete-recurring).
// New habit tasks ARE created via POST so they appear in the database.
...
export function diffTasks(serverTasks, editorTasks) {
  ...
  // avoid being re-created as duplicates.
  ...
  for (const editorTask of editorTasks) {
    ...
    if (serverMatch) {
      ...
      // Habit/recurring tasks: allow text edits (renames) and chip repairs,
      // but skip done-state changes (handled by pre-diff toggle in handleUpdate).
      // Virtual recurring appearances from other dates are never updated.
      if (isHabitOrRecurring(editorTask) || isHabitOrRecurring(serverMatch)) {
        // Never update virtual recurring appearances from other dates
        if (serverMatch._source === 'recurring') continue;
        ...
        const textChanged = cleanTaskText(editorTask.text) !== cleanTaskText(serverMatch.text);
        const chipRepair = editorHasChip && !serverHasChip;

        if (textChanged || chipRepair) {
          toUpdate.push({
            id: serverMatch.id,
            text: editorTask.text,
            html: editorTask.html,
            done: serverMatch.done, // preserve server done state (toggles handled elsewhere)
  ...
  // Unmatched server tasks → deleted.
  // Virtual recurring appearances (_source === 'recurring') are never deleted —
  // they don't belong to this date. Own-date habits CAN be deleted (user removed
  // the task from the editor = intent to delete the template everywhere).
  for (const serverTask of serverTasks) {
    if (matched.has(serverTask.id)) continue;
    if (serverTask._source === 'recurring') continue; // virtual, not ours to delete
...
export async function applyDiff(date, diff, token) {
  ...
  // Delete removed tasks — habit/recurring templates use ?recurring=1 to cascade
