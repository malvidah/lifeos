...
//
// IMPORTANT: Habit and recurring tasks are EXCLUDED from the diff.
// They have their own CRUD paths (complete-recurring, HabitsCard toggle).
// The diff only handles regular tasks to avoid duplicate/race issues.
...
function isHabitOrRecurring(task) {
...
export function diffTasks(serverTasks, editorTasks) {
  ...
  // Filter out habit/recurring tasks from both sides
  const regularServer = serverTasks.filter(t => !isHabitOrRecurring(t));
  const regularEditor = editorTasks.filter(t => !isHabitOrRecurring(t));
