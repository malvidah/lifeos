...
export function diffTasks(serverTasks, editorTasks) {
  ...
  for (const editorTask of editorTasks) {
    ...
    if (serverMatch) {
      ...
      // Recurring templates are read-only from non-origin dates.
      // Skip format-only text normalization (e.g. "/h daily" vs "{h:daily:Daily}").
      // Only track done changes (routed to complete-recurring by applyDiff).
      if (serverMatch._source === 'recurring') {
        const meaningfulTextChange = textChanged && cleanTaskText(editorTask.text) !== cleanTaskText(serverMatch.text);
        if (!doneChanged && !meaningfulTextChange) continue;
      }
