...
export function diffTasks(serverTasks, editorTasks) {
  ...
  for (const editorTask of editorTasks) {
    ...
    //    like "/h tr" vs "{h:tr:T·R}" for newly created tasks without data-task-id
    if (!serverMatch) {
    ...
    if (serverMatch) {
      ...
      // Recurring templates: skip only pure format normalization (where the
      // normalized text is identical, e.g. "/h daily" vs "{h:daily:Daily}").
      // Allow through any actual user edit (norm'd text differs).
      if (serverMatch._source === 'recurring') {
        if (!doneChanged && !textChanged) continue;
        // If text changed but norm() is identical, it's just format noise — skip
        if (textChanged && !doneChanged && norm(editorTask.text) === norm(serverMatch.text)) continue;
      }
      ...
      if (textChanged || doneChanged || posChanged) {
        toUpdate.push({
          ...
          _hasRecurrenceAttr: !!(serverMatch.html?.includes('data-recurrence=') || serverMatch.html?.includes('data-habit=')) && !serverMatch.html?.includes('data-completion="true"'),
  ...
  for (const serverTask of serverTasks) {
    if (!matched.has(serverTask.id)) {
      if (serverTask._source === 'recurring') {
        // Recurring template was removed from editor — user wants it deleted everywhere.
        // Mark it so applyDiff can call DELETE with the recurring flag.
        toDelete.push({ ...serverTask, _isRecurringTemplate: true });
      } else {
...
export async function applyDiff(date, diff, token) {
  ...
  for (const task of diff.toDelete) {
    ...
    if (task._isRecurringTemplate) params.set('recurring', '1');
    promises.push(
      api.delete(`/api/tasks?${params.toString()}`, token)
    );
  }
