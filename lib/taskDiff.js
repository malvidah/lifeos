...
export function diffTasks(serverTasks, editorTasks) {
  ...
  for (const et of editorTasks) {
    ...
    if (server) {
      ...
      // Also catch chip-only changes (e.g. swapping {h:1pw:1/week} for {h:5pw:5/week}):
      // cleanTaskText strips the chip token, so textChanged misses them — fall back
      // to raw text comparison.
      if (isHabit) {
        const rawTextChanged = et.text !== server.text;
        if (textChanged || rawTextChanged) {
          toUpdate.push({
            id: server.id,
            text: et.text,
            html: et.html,
