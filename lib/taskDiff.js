    ...
    if (serverMatch) {
      ...
      if (textChanged || doneChanged || posChanged) {
        toUpdate.push({
          ...
          // Carry source info for routing done changes on recurring tasks
...
export async function applyDiff(date, diff, token) {
  ...
  // Create new tasks — send the original HTML from the editor to preserve chips
  for (const task of diff.toCreate) {
    promises.push(
      api.post('/api/tasks', {
        date,
        text: task.text,
        html: task.html, // Preserve the editor's HTML with data-recurrence, data-project-tag, etc.
  ...
  // Update changed tasks — send HTML to preserve chips
  for (const task of diff.toUpdate) {
    // If a recurring/habit template is being checked off, use complete-recurring
    // instead of PATCH (which would corrupt the template)
    if (task._doneChanged && task.done && task._source === 'recurring') {
      promises.push(
    ...
    } else if (task._doneChanged && !task.done && task._source === 'recurring') {
      // Unchecking a recurring task — delete the completion row if one exists
      // (the PATCH guard will reject done=false on the template anyway)
      // Just skip — the template stays undone, completion row stays
      continue;
    } else {
      promises.push(
        api.patch('/api/tasks', {
