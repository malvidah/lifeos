...
export function diffTasks(serverTasks, editorTasks) {
  ...
  for (const editorTask of editorTasks) {
    // Habit/recurring tasks that already exist on the server (have a task_id)
    // are fully managed by their own CRUD paths (complete-recurring, HabitsCard).
    // Skip them entirely — they're not in regularServer indexes so they'd
    // wrongly fall through to toCreate and generate duplicates on every save.
    ...
    if (serverMatch) {
      ...
      // No match — this is a new task (including new habits without a task_id)
