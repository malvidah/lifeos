    ...
    if (serverMatch) {
      ...
      if (textChanged || doneChanged || posChanged) {
        toUpdate.push({
          ...
          // Carry source info so applyDiff can route recurring tasks correctly
...
export async function applyDiff(date, diff, token) {
  ...
  for (const task of diff.toUpdate) {
    // Recurring/habit task checkbox toggle → route to complete-recurring
    if (task._doneChanged && task._source === 'recurring') {
