...
export function diffTasks(serverTasks, editorTasks) {
  ...
  // Filter out habit/recurring tasks from server side (skip matching/deleting)
  const regularServer = serverTasks.filter(t => !isHabitOrRecurring(t));

  // Split editor tasks: habits go to create-only path, regular go through full diff
  const regularEditor = [];
  const newHabitEditor = [];
  for (const t of editorTasks) {
    if (isHabitOrRecurring(t)) {
      // Only CREATE new habit tasks (no task_id = not yet on server)
      if (!t.task_id && t.text?.trim()) {
        newHabitEditor.push(t);
      }
      // Existing habit tasks (with task_id) are managed by HabitsCard/complete-recurring
    } else {
      regularEditor.push(t);
    }
  }
  ...
  for (const editorTask of regularEditor) {
  ...
  // New habit tasks — POST them so they appear in the database
  for (const t of newHabitEditor) {
    toCreate.push(t);
  }
