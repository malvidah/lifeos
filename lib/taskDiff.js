...
export async function applyDiff(date, diff, token) {
  ...
  for (const task of diff.toUpdate) {
    ...
    // Handle recurring/habit tasks specially — only process done changes,
    // skip all other changes (text, position) to protect template integrity.
    if (task._hasRecurrenceAttr) {
      if (task._doneChanged && task.done && !task._serverDone) {
      ...
      // Skip all other changes on recurring/habit tasks (text edits, position)
      // to prevent accidentally stripping chips or corrupting templates
      continue;
    }
    promises.push(
      api.patch('/api/tasks', {
