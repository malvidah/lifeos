    ...
    if (serverMatch) {
      ...
      if (textChanged || doneChanged || posChanged) {
        toUpdate.push({
          ...
          // Carry info so applyDiff can route recurring/habit tasks correctly.
          // Check both _source AND the HTML for recurrence/habit attributes,
          // because own tasks created today have _source:'own' but still
          // need to go through complete-recurring if they're templates.
          _source: serverMatch._source,
          _isTemplate: serverMatch._source === 'recurring' ||
            !!(serverMatch.html?.includes('data-recurrence=') || serverMatch.html?.includes('data-habit=')),
          _doneChanged: doneChanged,
          _position: serverMatch.position,
        });
      }
    } else {
...
export async function applyDiff(date, diff, token) {
  ...
  for (const task of diff.toUpdate) {
    ...
    // Applies to recurring virtual appearances (_source:'recurring') AND
    // own tasks that are templates (have data-recurrence or data-habit in HTML).
    if (task._doneChanged && task._isTemplate) {
      hadRecurringDone = true;
      if (task.done) {
        // Checking off → create completion row
        promises.push(
      ...
      // Unchecking → skip (template should stay undone, completion row
      // will be absent on next reload since GET only shows it if done=true)
      continue;
    }
    promises.push(
      api.patch('/api/tasks', {
