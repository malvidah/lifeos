...
export function diffTasks(serverTasks, editorTasks) {
  ...
  for (const st of serverTasks) {
    ...
    // Recurring/habit instances that vanished from the editor = user wants to delete the template.
    // isHabitOrRecurring catches _source==='recurring' and adds &recurring=1 for cascade deletion.
    toDelete.push(st);
  }
