...
export function parseNaturalDate(query) {
  ...
  // yesterday / yest
  if (q === 'yesterday' || q === 'yest') return toKey(shift(today, -1));
  ...
  // last week / lw
  if (q === 'last week' || q === 'lw') return toKey(shift(today, -7));

  // in N days / in N weeks
  const inMatch = q.match(/^in\s+(\d+)\s+(day|days|week|weeks)$/);
  if (inMatch) {
    const n = parseInt(inMatch[1], 10);
    const unit = inMatch[2].startsWith('week') ? 7 : 1;
    return toKey(shift(today, n * unit));
  }

  // N days ago / N weeks ago
  const agoMatch = q.match(/^(\d+)\s+(day|days|week|weeks)\s+ago$/);
  if (agoMatch) {
  ...
  // "next <day>" — e.g. "next friday", "next fri"
  const nextDayMatch = q.match(/^next\s+(.+)$/);
  if (nextDayMatch) {
    const dayQ = nextDayMatch[1];
    for (let i = 0; i < 7; i++) {
      if (DAYS_FULL[i] === dayQ || DAYS_ABBR[i] === dayQ) {
  ...
  // "last <day>" — e.g. "last monday", "last mon"
  const lastDayMatch = q.match(/^last\s+(.+)$/);
  if (lastDayMatch) {
    const dayQ = lastDayMatch[1];
    for (let i = 0; i < 7; i++) {
      if (DAYS_FULL[i] === dayQ || DAYS_ABBR[i] === dayQ) {
        const dow = today.getDay();
        let diff = dow - i;
  ...
  if (mdMatch) {
    ...
    if (monthIdx >= 0 && day >= 1 && day <= 31) {
      ...
      // Don't auto-bump to next year — allow past dates
...
export function generateDateSuggestions(query) {
  ...
  if (!query || !query.trim()) {
    ...
    return [
      { label: 'Yesterday',   date: toKey(shift(today, -1)) },
  ...
  if ('yesterday'.startsWith(q)) {
    results.push({ label: 'Yesterday', date: toKey(shift(today, -1)) });
  }
  // "tom..." → Tomorrow
  if ('tomorrow'.startsWith(q) && q.length > 1) {
    results.push({ label: 'Tomorrow', date: toKey(shift(today, 1)) });
  }
  ...
  // "next ..." → show next <day> options
  if (q.startsWith('next')) {
    const rest = q.slice(4).trim();
    if (!rest) {
      // Show all "next" options
      results.push({ label: 'Next Week', date: toKey(shift(today, 7)) });
      for (let i = 0; i < 7; i++) {
        const dow = today.getDay();
        let diff = i - dow;
        if (diff <= 0) diff += 7;
        results.push({ label: 'Next ' + DAYS_FULL[i].charAt(0).toUpperCase() + DAYS_FULL[i].slice(1), date: toKey(shift(today, diff)) });
      }
    } else {
      if ('week'.startsWith(rest)) results.push({ label: 'Next Week', date: toKey(shift(today, 7)) });
      for (let i = 0; i < 7; i++) {
        if (DAYS_FULL[i].startsWith(rest) || DAYS_ABBR[i].startsWith(rest)) {
          const dow = today.getDay();
          let diff = i - dow;
          if (diff <= 0) diff += 7;
          results.push({ label: 'Next ' + DAYS_FULL[i].charAt(0).toUpperCase() + DAYS_FULL[i].slice(1), date: toKey(shift(today, diff)) });
  ...
  // "last ..." → show last <day> options
  if (q.startsWith('last')) {
    const rest = q.slice(4).trim();
    if (!rest) {
      results.push({ label: 'Last Week', date: toKey(shift(today, -7)) });
      for (let i = 0; i < 7; i++) {
        const dow = today.getDay();
        let diff = dow - i;
        if (diff <= 0) diff += 7;
        results.push({ label: 'Last ' + DAYS_FULL[i].charAt(0).toUpperCase() + DAYS_FULL[i].slice(1), date: toKey(shift(today, -diff)) });
      }
    } else {
      if ('week'.startsWith(rest)) results.push({ label: 'Last Week', date: toKey(shift(today, -7)) });
      for (let i = 0; i < 7; i++) {
        if (DAYS_FULL[i].startsWith(rest) || DAYS_ABBR[i].startsWith(rest)) {
          const dow = today.getDay();
          let diff = dow - i;
