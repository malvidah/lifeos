...
export function useDbSave(date, type, empty, token, userId) {
    ...
    const flush = () => {
      ...
      if (pendingRef.current !== null && pendingRef.current !== lastSavedRef.current) {
        ...
        if (v !== lastSavedRef.current) {
          ...
          dbSave(dateRef.current, type, v, token);
        }
        clearDirty(cacheKey);
      }
    };
  ...
  const setValue = useCallback((u, { undoLabel, skipHistory } = {}) => {
    ...
    pendingRef.current = null;             // clear pending — we're saving now
    setMemValue(cacheKey, next);           // reactive update via store
    // Dedup: skip DB save if value is identical to the last save (prevents
    // double-save from onBlur + unmount flush racing on the same content).
