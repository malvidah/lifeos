...
export function useDbSave(date, type, empty, token, userId) {
  ...
  useEffect(() => {
    ...
    // Already in cache and not dirty — use it (data was loaded previously)
    ...
    if (!isOnline()) {
      if (cacheKey in getStore().mem) markHydrated(cacheKey); // localStorage-cached data counts as hydrated
      else loadMemValue(cacheKey, empty); // truly empty — NOT hydrated, so empty writes will be blocked
      return;
    }
