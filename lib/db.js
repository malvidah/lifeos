...
export async function dbSave(date, type, data, token) {
  ...
  if (TYPED_ENDPOINTS[type]) {
    ...
    // Guard 2: never write empty values — protects against empty cache overwriting real DB data.
    const isEmpty =
      data === null || data === undefined || data === "" ||
      (typeof data === "object" && !Array.isArray(data) && Object.keys(data).length === 0) ||
      (Array.isArray(data) && data.length === 0);
    if (isEmpty) {
      console.warn("[dbSave] Skipping empty write — won't overwrite server data", { date, type });
      return;
    }
  }
...
function dbSaveBeacon(date, type, data, token) {
  if (!token) return;
  // Same empty guard as dbSave — never overwrite server data with nothing
  const isEmpty =
    data === null || data === undefined || data === "" ||
    (typeof data === "object" && !Array.isArray(data) && Object.keys(data).length === 0) ||
    (Array.isArray(data) && data.length === 0);
