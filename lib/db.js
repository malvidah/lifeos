...
export function useDbSave(date, type, empty, token, userId) {
  ...
  useEffect(() => {
    ...
    // If cached (from localStorage) and first load, show cached value immediately
    // but still fetch from server to get fresh data
    if (cacheKey in mem && !dirty[cacheKey] && rev === 0) {
      markHydrated(cacheKey);
      // Fall through to fetch fresh data from server below
    }
