...
export function useDbSave(date, type, empty, token, userId) {
  ...
  // Without this, the flush handler (which reads dateRef.current) would save old content
  // to the new date — causing journal entries to leak across days.
  if (dateRef.current !== date) {
    if (pendingRef.current !== null && pendingRef.current !== lastSavedRef.current) {
      // Fire-and-forget save to the OLD date before we lose the reference
      dbSave(dateRef.current, type, pendingRef.current, token);
  ...
  const setValue = useCallback((u, { undoLabel, skipHistory } = {}) => {
    ...
    if (next !== lastSavedRef.current) {
      // Defer the fetch so that if the browser is unloading, the flush
      // handler can cancel this timer and use sendBeacon instead.
      // 300ms debounce prevents concurrent POST requests from racing
      // (each POST does DELETE+INSERT; overlapping POSTs duplicate rows).
